...
from forms import (UserEditForm, UserListForm, UserLoginForm,
                   UserMealCalendarForm, UserMealCalenderDateForm,
                   UserRegisterForm)
from models import Calendar, List, Meal, User, connect_db, db
...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get(
    "DATABASE_URL", "postgresql:///meal_planning_db"
)
...
def search_engine(user_id):
    ...
    if check_user:
        ...
        return render_template(
            "search.html", data=data["meals"], search_input=search_input
        )
    else:
        return redirect("/")
...
def add_own_meal(user_id):
    ...
    if check_user:
        ...
        if form.validate_on_submit():
            ...
            dateInfo = Calendar(
                user_id=user_id, meal_name=meal_name, selected_date=dateSelected
            )
        ...
        meals = (
            Calendar.query.filter(
                Calendar.selected_date.like(
                    f"{request.json['year']}-{request.json['month']}-%"
                )
            )
...
@app.route(
    "/users/<int:user_id>/calendar/add/<int:meal_id>/<meal_name>",
    methods=["GET", "POST"],
)
...
def shopping_list(user_id):
    ...
    if check_user:
        ...
        if form.validate_on_submit():
            ...
            return render_template(
                "user_shopping_list.html", form=form, todo_list=todo_list
            )
    else:
        return redirect("/")
...
def meal_info(user_id, meal_id, meal_name):
    ...
    if check_user:
        ...
        data = res.json()
        saved_meal = Meal.query.filter(
            Meal.meal_id == meal_id, Meal.user_id == user_id
        ).one_or_none()
        ingredients = []
        measure = []
...
@app.route(
    "/users/<int:user_id>/meals/<int:meal_id>/view/<meal_name>", methods=["POST"]
)

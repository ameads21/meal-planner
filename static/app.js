...
async function saveMeal(evt) {
  ...
  await axios.post(
    `/users/${data.user_id}/meals/${data.meal_id}/view/${data.meal_name}`,
    data
  );
...
async function genearateCalendar() {
  $(".calendarDiv").html(
    '<div class="d-flex justify-content-center spinner"><div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div></div>'
  );
  data = {
    month: month,
    year: year,
...
function addData(data) {
  ...
  for (d of data) {
    ...
    if (d.meal_id != null) {
      $(
        `<li class="meal_info list-group-item text-center"><button class="close meal_info list-group-item " id="meal_${numMeals}" onclick="delete_calendar_meal(${d.user_id},${d.id}, '${d.date}', ${numMeals})" aria-label="Close"><span aria-hidden="true">&times;</span></button><a href="/users/${d.user_id}/meals/${d.meal_id}/view/${d.meal_name}">${d.meal_name}</a></li>`
      ).appendTo($(`#day-${Number(day)}`));
    } else {
      $(
        `<li class="meal_info list-group-item text-center"><button class="close" id="meal_${numMeals}" onclick="delete_calendar_meal(${d.user_id},${d.id},'${d.date}', ${numMeals})" aria-label="Close"><span aria-hidden="true">&times;</span></button>${d.meal_name}</li>`
      ).appendTo($(`#day-${Number(day)}`));
    }
    numMeals += 1;
  }
